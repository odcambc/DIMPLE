...
parser.add_argument(
    "-fragmentLen", default=[], type=int, help="Maximum length of gene fragment"
)
parser.add_argument(
    "-overlap",
    default=3,
    type=int,
    help="Enter number of bases to extend each fragment for overlap. This will help with insertions close to fragment boundary",
)
parser.add_argument(
    "-DMS",
    action="store_const",
...
parser.add_argument(
    ...
    default=False,
    nargs="+",
    help="Enter a list of insertions (nucleotides) to make at every position. Note, you should enter multiples of 3 nucleotides to maintain reading frame",
)
parser.add_argument(
    "-deletions",
    default=False,
...
parser.add_argument(
    "-barcode_start",
    default=0,
    help="To run DIMPLE multiple times, you will need to avoid using the same barcodes. This allows you to start at a different barcode.",
)
parser.add_argument(
    "-restriction_sequence",
    default="CGTCTC",
    help="Recommended using BsmBI - CGTCTC or BsaI - GGTCTC",
)
parser.add_argument(
    "-avoid_sequence",
    nargs="+",
    default=["CGTCTC", "GGTCTC"],
    help="Avoid these sequences in the backbone - BsaI and BsmBI. For multiple sequnces use a space between inputs. Example -avoid_sequence CGTCTC GGTCTC",
)
parser.add_argument(
    "-include_stop_codons",
