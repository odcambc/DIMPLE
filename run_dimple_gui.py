...
def run():
    ...
    if app.fragmentLen.get() != "auto":
        ...
        DIMPLE.maxfrag = (
            int(app.oligoLen.get()) - 62 - overlapL - overlapR
...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        tk.Label(
            self, text="Fragment Overlap (This will change if deletions are selected)"
        ).pack()
        ...
        self.restriction_sequence = tk.Entry(
            self, textvariable=tk.StringVar(self, "CGTCTC")
        )
        ...
        self.delete_check = tk.Checkbutton(
            self, text="List of Deletions", variable=self.delete, command=DMS_ON
        )
        ...
        self.insert_check = tk.Checkbutton(
            self, text="List of Insertions", variable=self.insert, command=DMS_ON
        )
        self.insert_check.pack()
        self.insert_check.deselect()
        self.insertions = tk.Entry(
            self, width=50, textvariable=tk.StringVar(self, "GGG,GGGGGG")
        )
        self.insertions.pack()

        self.include_sub_check = tk.Checkbutton(
            self, text="Include Substitutions", variable=self.include_substitutions
        )
